...
ALTER TABLE customers
  ALTER COLUMN balance_cents TYPE numeric(12,2) USING (balance_cents / 100.0) AS money;
...
UPDATE customers
SET account_no = 'ACC-' || EXTRACT(YEAR FROM registered_at)::text || '-' || LPAD(id::text, 5, '0');
...
-- Задача 5
ALTER TABLE customers ADD CONSTRAINT status_valid CHECK (status IN ('active', 'blocked', 'pending'));
